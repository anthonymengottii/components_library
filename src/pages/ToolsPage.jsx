        ...
        >
          <Flex align="center" gap={2.5}>
            <Flex
              ...
              boxShadow={`0 2px 8px ${colors.brandAlpha10}`}
            >
        ...
        {TOOLS.map(tool => (
          ...
          >
            <Flex
              ...
              h={6}
              borderRadius="8px"
              bg={selectedTool === tool.id ? `linear-gradient(135deg, ${colors.primary} 0%, var(--color-accent) 100%)` : colors.bgHover}
              transition="all 0.15s"
            >
...
const ComingSoon = ({ label, toolSelector, t }) => (
...
export default function ToolsPage() {
  ...
  useEffect(() => {
    if (!toolId) {
      document.title = `Components Library - ${t('tools.title')}`;
    } else {
      const tool = TOOLS.find(t => t.id === toolId);
      if (tool) {
    ...
    return (
      <>
        ...
        <section className="tools-landing">
          <title>Components Library - {t('tools.title')}</title>

          <div className="tools-landing-header">
            <div className="tools-landing-header-left">
              <h1 className="tools-landing-title">{t('tools.title')}</h1>
                  ...
                  <div className="tools-landing-card-body">
                    <div className="tools-landing-card-top">
                      <div className="tools-landing-card-title-row">
                        ...
                        {!tool.component && (
                          <span className="tools-landing-card-tag soon">{t('tools.comingSoon')}</span>
                        )}
                      </div>
  ...
  return (
    <Box h="100vh" bg="var(--bg-body)" overflow="hidden">
      <Navbar showDocs />
